...
RECORDNAME has to match in full, so 'FOV' never matches ':FOV_OFFSETS', and of the records under one RECORDNAME 
the first one keyed as asked is the one read, so the key 'tr' never picks up ':VAR tramp, 100'.
"""
...
class MRSdata:
    ...
    def __init__(self):
        ...
        self.nswitches = 1              # a divisor downstream, never 0
        self.npoints_per_switch = 0     # number of points to acquire per switch
    ...
    def probe_from_buffer(self, fdbytes):
        """
    ...
    def _set_parameters(self):
        ...
        FIELDS = (
            ...
            ('nswitches', 'VAR', 'no_switches', int, 1),                          # ':VAR no_switches, 64'
        ...
        for attr, fieldname, key, dtype, scale in FIELDS:
            ...
            try:
                ...
                print(f"   {reason}, keeping {attr}={getattr(self, attr)}", file=sys.stderr)
                continue
