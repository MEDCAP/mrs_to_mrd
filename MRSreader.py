...
':RECORDNAME ...\\r\\n' records. There are two notations for a parameter:
    ...
    pattern1: RECORDNAME value              :FOV 45
                                            :AcquisitionStartTime 13312456532890

    pattern2: RECORDNAME key, value         :OBSERVE_FREQUENCY "13C 0.0", 0.0, MHz, kHz, Hz, rx1MHz
                                            :VAR alpha, 13
                                            :SAMPLE_PERIOD sample_period, 400, 14, "25.0 KHz  40 us"

A parameter is looked up by its RECORDNAME, and by the key as well wherever the record carries one -
the sequence variables are all recorded as ':VAR', ~50 of them in one scan, and the base frequency
record is keyed by nucleus. RECORDNAME has to match in full, so 'FOV' never matches ':FOV_OFFSETS',
and of the records under one RECORDNAME the first one keyed as asked is the one read, so the key 'tr'
...
class MRSdata:
    ...
    DATA_START = 512
    # number of bytes one acquired point occupies, per MR Solutions data format code. Every code but 3 is
    ...
    def __init__(self):
        ...
        self.base_frequency_in_SPR = False  # set when the header defers the frequency to the .SPR file in the same directory
        self.sequence_name = ''
        self.sample_period = 0          # in 100ns
        self.acquisition_timestamp = 0  # in 100ns since some epoch time but this gets reset every bootup
        self.flip_angle = 0             # in degrees
        self.naverages = 0              # MRS aggregates the averages dimension 
        self.nswitch = 1                # a divisor downstream, never 0
        self.npoints_per_switch = 0     # number pof points to acquire per switch
        ...
        self.quiet = False              # warn if the record to search in the header is missing. Only raise errors when single file is read
    ...
    def read_from_file(self, filepath):
        ...
        if self.base_frequency_in_SPR:
            # look for 'FREQ' record in .SRP file, otherwise, keep it as default 
    ...
    def _set_parameters(self):
        """
        Extract value from header based on RECORDNAME notated as two patterns below:
        pattern1: RECORDNAME value               :FOV 45
                                                :AcquisitionStartTime 13312456532890
        ...
        """
        # sequence_name are stored with RECORDNAME 'SEQUENCE' for EVO2 and 'PPL' for EVO1
        ...
        # base_frequency are stored with RECORDNAME='FREQUENCY' for EVO2 and 'OBSERVE_FREQUENCY' for EVO1
...
if __name__ == '__main__':
    ...
    print(f"{mrs.rawdata.shape if mrs.rawdata is not None else 'no data'} "
          f"nsamples x nviews x nsliceviews x nslices x nechoes x nrepetitions")
