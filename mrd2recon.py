...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Reconstruct MRS data from raw.mrd2 file to recon.mrd2 file. Specify metabolite peaks with -<name> <offset in ppm>. Add * after name to indicate that this peak is definitely small and should not be used to identify the largest peak. Specify -f <basedir> to run all raw.mrd2 files in a directory tree. Specify -n <string> to only run files with <string> in the name. Specify -w <wigglefactor> to set the wiggle factor for lorn fitting (default 1.0).')
    parser.add_argument('-f', '--folder', type=str, required=True, help='base directory to search for raw.mrd2 files')
    parser.add_argument('-w', '--wiggle', type=float, required=False, default=1.0, help='wiggle factor for lorn fitting (default 1.0)')
    parser.add_argument('-n', '--name', type=str, required=False, default='', help='run the script with this filename included')
    parser.add_argument('-p', '--peak', type=str, required=False, help='specify metabolite peaks with quotes -<name> <offset in ppm>. Add * after name to indicate that this peak is definitely small and should not be used to identify the largest peak. Example: -bic* 0.0 -urea 2.3 -pyr 9.7 -ala* 15.2 -hyd* 18.1 -lac 21.8')
    args = parser.parse_args()

    basedir = args.folder
    wiggle = args.wiggle
    targetfiletype = args.name
    peakoffsets = []
    peaknames = []
    biggestpeakidx = []
    peakargs = args.peak.split(' ')
    for i, item in enumerate(peakargs):        
        if item[0] == '-' and item[-1] == '*':
            peaknames.append(item[1:-1])
            peakoffsets.append(float(peakargs[i + 1]))
        elif item[0] == '-':
